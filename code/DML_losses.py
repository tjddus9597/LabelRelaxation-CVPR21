...
class TripletLoss(nn.Module):
    def __init__(self, margin=0.2, **kwargs):
        ...
        self.margin = margin
        self.miner = miners.TripletMarginMiner(margin, type_of_triplets = 'DistanceWeightedMiner')
...
class CrossEntropy(nn.Module):
    def __init__(self, nb_classes, sz_embed, temperature):
        super(CrossEntropy, self).__init__()
        self.temperature = temperature
        self.classifier = nn.Linear(sz_embed, nb_classes).cuda()
        self.cross_entropy = nn.CrossEntropyLoss(reduction='none')
        
    def forward(self, X, T):
        logit = self.classifier(X) / self.temperature
        loss = self.cross_entropy(logit, T).mean()
        return loss